...
| 指令                        | 事例                                              | 内容                     | 其他说明                                                                                               |
| --------------------------- | ------------------------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------ |
| `yum [-y] [install          | remove                                            | search] softname`        | `yum install wget`                                                                                     | CentOS使用yum管理器 | 需要root权限；选项：-y，自动确认，无需手动确认安装或卸载过程；install：安装；remove：卸载；search：搜索 |
| `apt [-y] [install          | remove                                            | search] softname`        | `apt -y install wget`                                                                                  | Ubuntu使用apt管理器 | ^                                                                                                       |
| `systemctl start            | stop                                              | status                   | enable                                                                                                 | disable 服务名`     | `systemctl start nginx`                                                                                 | 使用systemctl命令控制软件的启动和关闭;部分软件安装后没有自动集成到systemctl中，需要手动添加 | start 启动;stop 关闭;status 查看状态;enable 开启开机自启;disable 关闭开机自启 |
| `ln -s path1 path2`         | `ln -s /etc/yum.conf ~/yum.conf`                  | 创建软链接               | -s选项，创建软连接;path1：被链接的文件或文件夹;path2：要链接去的目的地                                 |
